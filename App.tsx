...
import { Send, Mic, Upload, LogOut, Book, Image as ImageIcon, Layout, FileText, Plus, X, Brain, Volume2, VolumeX, Pin, Edit2, Trash2, Eye, EyeOff, Loader2, Maximize2, Menu, Info } from 'lucide-react';
...
const App: React.FC = () => {
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6 scrollbar-thin scrollbar-thumb-violet-200">
            ...
            <div>
                ...
                <div className="space-y-2">
                    {sessions.map(s => (
                        <div key={s.id} className={`group flex items-center gap-2 p-3 rounded-lg cursor-pointer transition-all border ${session?.id === s.id ? 'bg-violet-50 border-violet-200 shadow-sm' : 'hover:bg-white border-transparent'}`} onClick={() => { setSession(s); setShowMobileMenu(false); }}>
  ...
  const VisualsContent = () => (
    <div className="flex flex-col h-full gap-6">
        ...
        <div>
             <button 
                onClick={() => { setIsMapOpen(true); setShowMobileVisuals(false); }}
                className="w-full bg-white shadow-md border border-scholar-blue/20 hover:border-scholar-blue text-scholar-blue font-bold py-3 px-4 rounded-xl flex items-center justify-between transition-all group"
             >
        ...
        {/* Illustrator Section */}
        <div className="flex-1 flex flex-col overflow-hidden bg-slate-50 rounded-2xl border border-slate-100">
    ...
    <div className="flex h-screen bg-gradient-to-br from-indigo-50 via-white to-purple-50 text-slate-800 overflow-hidden font-sans">
      ...
      {showMobileMenu && (
        <div className="fixed inset-0 z-50 flex">
            <div className="w-72 h-full bg-white/95 backdrop-blur shadow-2xl animate-slide-in-left">
                <SidebarContent />
            </div>
            <div className="flex-1 bg-slate-900/50 backdrop-blur-sm" onClick={() => setShowMobileMenu(false)} />
        </div>
      )}

      {/* MOBILE VISUALS OVERLAY */}
      {showMobileVisuals && (
        <div className="fixed inset-0 z-50 flex justify-end">
            <div className="flex-1 bg-slate-900/50 backdrop-blur-sm" onClick={() => setShowMobileVisuals(false)} />
            <div className="w-80 h-full bg-white/95 backdrop-blur shadow-2xl p-6 animate-slide-in-right flex flex-col">
                <div className="flex justify-end mb-4">
                     <button onClick={() => setShowMobileVisuals(false)} className="p-2 bg-slate-100 rounded-full hover:bg-slate-200"><X size={20}/></button>
                </div>
                <VisualsContent />
            </div>
        </div>
      ...
      <aside className="w-72 bg-white/80 backdrop-blur-md border-r border-violet-100 hidden md:flex flex-col z-10 shadow-sm">
         <SidebarContent />
      </aside>

      {/* CENTER - CHAT OR NEXUS */}
      <main className="flex-1 flex flex-col relative z-0 w-full min-w-0">
        {/* Header */}
        <header className="h-16 md:h-20 bg-white/60 backdrop-blur-md border-b border-violet-100 flex items-center justify-between px-4 md:px-8 shadow-sm">
            <div className="flex items-center gap-3 overflow-hidden">
                {/* Mobile Menu Trigger */}
                <button onClick={() => setShowMobileMenu(true)} className="md:hidden p-2 -ml-2 text-slate-600 hover:bg-slate-100 rounded-full">
                    <Menu size={24} />
                </button>

                <h2 className="font-bold text-slate-700 flex items-center gap-2 md:gap-3 text-lg md:text-xl truncate">
                    {view === 'nexus' ? <Layout size={24} className="text-scholar-violet flex-shrink-0"/> : <Book size={24} className="text-scholar-violet flex-shrink-0"/>}
                    <span className="truncate">{view === 'nexus' ? 'Agent Nexus' : session?.topic}</span>
                </h2>
            </div>

            <div className="flex items-center gap-2 md:gap-4">
                 {/* Mobile Visuals Trigger */}
                 <button 
                    onClick={() => setShowMobileVisuals(true)} 
                    className="lg:hidden p-2 text-scholar-blue hover:bg-blue-50 rounded-full"
                    title="View Visuals & Map"
                 >
                     <ImageIcon size={24} />
                 </button>
                 ...
                 <button 
                    ...
                    className={`flex items-center gap-2 px-3 md:px-4 py-2 rounded-full text-sm font-medium transition-all ${isNarrationOn ? 'bg-scholar-violet/10 text-scholar-violet' : 'text-slate-400 hover:bg-slate-100'}`}
                ...
                <div className="flex bg-slate-100/80 p-1 rounded-lg">
                    <button onClick={() => setView('study')} className={`px-3 md:px-6 py-1.5 md:py-2 text-sm md:text-base font-medium rounded-md transition-all ${view === 'study' ? 'bg-white shadow-sm text-scholar-darkViolet' : 'text-slate-500 hover:text-slate-700'}`}>Study</button>
                    <button onClick={() => setView('nexus')} className={`px-3 md:px-6 py-1.5 md:py-2 text-sm md:text-base font-medium rounded-md transition-all ${view === 'nexus' ? 'bg-white shadow-sm text-scholar-darkViolet' : 'text-slate-500 hover:text-slate-700'}`}>Nexus</button>
                </div>
            </div>
        </header>
        ...
        {view === 'nexus' ? (
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
                <div className="flex-1 overflow-y-auto p-4 md:p-8 space-y-6 md:space-y-8 scrollbar-thin scrollbar-thumb-violet-200">
                    {session?.messages.map((msg) => (
                        <div key={msg.id} className={`flex ${msg.role === MessageRole.USER ? 'justify-end' : 'justify-start'}`}>
                            <div className={`max-w-[85%] md:max-w-[75%] rounded-2xl p-4 md:p-6 shadow-sm transition-all ${
                                ...
                                <p className="text-sm md:text-lg leading-6 md:leading-8 whitespace-pre-wrap font-medium">{msg.content}</p>
                                <span className={`text-xs mt-2 block opacity-80 ${msg.role === MessageRole.USER ? 'text-violet-200' : 'text-slate-400'}`}>
                ...
                <div className="p-4 md:p-6 bg-white/80 backdrop-blur border-t border-violet-100">
                    <div className="max-w-4xl mx-auto relative flex items-center gap-2 md:gap-4">
                        <input
                            ...
                            className="flex-1 bg-white border border-slate-200 rounded-full px-4 md:px-8 py-3 md:py-5 focus:outline-none focus:ring-2 focus:ring-scholar-violet/50 transition-all shadow-sm text-base md:text-lg"
                        />
                        <button 
                            onClick={toggleVoiceInput}
                            className={`p-3 md:p-5 rounded-full transition-all shadow-sm ${
                        ...
                        <button 
                            ...
                            disabled={!input.trim()}
                            className="p-3 md:p-5 bg-scholar-violet text-white rounded-full hover:bg-scholar-darkViolet disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-md transform hover:scale-105"
                        >
                            <Send size={20} className="md:w-6 md:h-6" />
                        </button>
                    </div>
                    {/* Active Agent Indicator */}
                    <div className="absolute top-[-15px] left-1/2 -translate-x-1/2 flex gap-3 pointer-events-none w-full justify-center px-4">
                         {agents.map(a => a.isActive && (
                             <span key={a.type} className="text-[10px] md:text-xs font-bold bg-scholar-mint/90 text-white px-3 py-1 rounded-full shadow-md animate-bounce border border-white truncate max-w-[150px]">
      ...
      <aside className="w-96 bg-white/80 backdrop-blur border-l border-violet-100 hidden lg:flex flex-col z-10 shadow-sm relative pt-6 px-6 pb-6">
        <VisualsContent />
      </aside>

      {/* ARCHITECT MAP MODAL */}
      {isMapOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 md:p-8 animate-fade-in">
              <div className="bg-white w-full max-w-6xl h-[90vh] md:h-[85vh] rounded-3xl shadow-2xl overflow-hidden flex flex-col relative animate-scale-up">
                  {/* Modal Header */}
                  <div className="h-16 border-b border-slate-100 flex items-center justify-between px-4 md:px-8 bg-slate-50/50">
                      <div className="flex items-center gap-3">
                          <Layout size={24} className="text-scholar-blue" />
                          <h3 className="text-lg md:text-xl font-bold text-slate-700">Knowledge Architecture</h3>
                      </div>
                      <button 
                  ...
                  {/* Map Content */}
                  <div className="flex-1 p-4 md:p-8 bg-slate-50/30 overflow-hidden">
                        {session?.mermaidCode ? (
